...
- **Avatar logging no longer runs for people who left the instance (hotfix).** Avatar lookups could continue for players who were no longer present — including after they left while Scarlet and VRChat kept running, and for players from a previous instance or a closed VRChat session whose state lingered in memory. Departed players deliberately remain visible in the player table, but Scarlet now stops following them entirely: leaving drops their name-to-id mapping so a later avatar-switch line can't re-register them, and every avatar path — the log-based `[API]` analysis handler, the database/API fallback, queued lookups, and the rescan sweep — checks presence before doing any work and before recording any result. A lookup still in flight when a player leaves has its result discarded rather than stored. Per-instance avatar state is also cleared when leaving an instance and when VRChat quits, and rescans refuse to run unless a VRChat process is live and an instance is current. Besides being the correct privacy behaviour, this removes a class of unnecessary avatar lookups and the rate-limit pressure that came with them.
