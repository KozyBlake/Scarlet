...
- **Multi-group support (experimental, opt-in).** One Scarlet process can now moderate several VRChat groups at once. Each group is a fully independent core with its own data folder, VRChat session, and Discord bot, shown as a tab in one shared window; the default single-group launch is unchanged (byte-identical). Multi-group is turned on by a single master **Multi-group mode** setting (Settings → Interface; restart to apply). When it is on, your existing group keeps running from the base data folder untouched (nothing is moved), and each subfolder of `groups/` adds another group. To keep it from overrunning VRChat's per-account limits, all cores on one account share a single API rate limiter (cooperative pacing instead of each core hammering independently and tripping 429s), and every Discord moderation post is tagged in its footer with the group it belongs to so moderators can tell actions apart even in a shared channel. Enabling the master setting pops a warning of the caveats first (declining leaves it off), and **File → Add group from this one** opens a short form that clones the current group's config into a new slot and gathers the new group's id up front (plus, when the per-account/per-token sub-toggles are on, its VRChat login and Discord bot token), so a new group is configured with a single restart instead of several. Credentials are pre-seeded as plaintext migration keys that the group's first login moves into its own secure store and clears; operational data (watch lists, session, audit history) is never copied. Groups are shown as tabs in one shared window (the frame-level menu bar swaps with the active tab; closing the window stops every group). Two credential sub-toggles let each group use its **own VRChat account** and its **own Discord bot token**: those secrets are routed to a per-group store (keyed by group id, with the shared store as a read fallback so existing single-group installs are unchanged), and per-account groups get their own API rate budget instead of sharing one. Still in progress: shared base config with per-group overrides. (A single shared bot token serving all guilds is a different, larger design not taken here — per-group tokens fit the one-connection-per-token rule more simply.) Running many busy groups on one account remains rate-limited by design; separate accounts per group are the way to scale further.
...
- **Multi-language desktop client (complete), now shipping English + Russian.** Building on the 0.4.18-b1 foundation, the **entire desktop UI is now translated** — splash, menu bar, every settings label/section header/enum value/theme-preset name, the instance-creation wizard, moderation confirmations and result popups, the avatar-stats panel, export/import and update dialogs, the status bar, and the spoken advisory/TTS callouts (~590 message keys, verified 1:1 between English and Russian). It follows the operating system language by default, with a `ui_language` override applied at startup. **English and Russian are complete**; German, Spanish, and Indonesian remain seeded pilots and fall back to English for anything not yet translated. The Discord bot still renders in English for now (its string migration is a separate pass), though each interaction already resolves against the invoking user's own Discord locale.
