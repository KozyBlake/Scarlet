...
- **Optional moderation-tag browser.** The existing search-first **Edit tags** flow remains the default. A separate **Browse tags** button offers a direct, ephemeral list of all moderation tags, split into up to five Discord-supported 25-option menus with the entry's existing tags preselected. Adapted from [Chloethecat's Scarlet fork](https://github.com/Chloethecat/Scarlet).
- **Desktop GitHub Actions build.** Relevant pushes to `main` now build vendored libdave and Scarlet on Java 21, then retain the desktop JAR as a 30-day workflow artifact. Contributed by [Chloethecat](https://github.com/Chloethecat).
...
- **First-launch "vibe-coded" disclosure.** On first run, Scarlet shows a one-time notice that this fork is maintained by **KozyBlake** and developed largely with AI assistance, with **Continue** or **Close Scarlet** — so anyone who'd rather not run an AI-assisted project can bow out up front. Shown once (persisted as `vibecoded_notice_acknowledged`); headless/scheduled runs log the notice once and continue instead of blocking on a dialog. A matching note sits at the top of the README. (KozyBlake maintains and is accountable for the project; the AI is a tool, not the maintainer.)
...
- **Reverse-image avatar lookup rebuilt without a keyed provider.** "Search by picture" used to depend solely on avtrDB. When this release was prepared, its endpoint was rejecting Scarlet's datacenter-hosted requests, so it is rebuilt from pieces Scarlet already trusts: the image's file ID is resolved to its owner via VRChat's own (authenticated) file API, each remaining provider is queried by author (the standard VRCX `authorId` lookup), and only the avatar whose image matches that file ID is kept. No new API key, no new third-party dependency. Author-lookup failures are tracked separately from text search, so a provider that doesn't support author lookup can't disable its text search. If the owner can't be resolved or no provider indexes them, it returns no match and falls back to name search, as before.

### Fixed

- **VRChat current-user responses no longer fail when nested avatar tags are arrays.** VRChat now returns `presence.currentAvatarTags` as an array in some responses while `vrchatapi-java` 1.21.0 expects a string. Scarlet normalizes only that mismatched nested field before deserialization. Adapted from [Chloethecat's Scarlet fork](https://github.com/Chloethecat/Scarlet).
...
- **ARM/glibc Discord voice (DAVE): groundwork + two protocol-version bugs.** On ARM/Termux, Discord voice can join/leave-thrash when the native DAVE library falls back to the JNA path. Two `(short) transitionId` casts that should have used the negotiated MLS protocol version were corrected, and the DAVE handshake path (`DAudioDaveSession`) now logs each epoch/transition step so a real ARM voice session can pinpoint where the handshake fails. ARM natives stay under `-android.jar`, out of the main desktop jar, by design. (Groundwork — the underlying glibc-aarch64 fallback still needs a test log from an affected user to close out.)
