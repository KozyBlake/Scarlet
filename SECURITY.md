...
| `peachpuff-swan-183728.hostingersite.com` (default; changeable via `mobile_relay_endpoint`) | Optional **relay**, run by this fork's maintainer, for delivering mobile alerts to a paired phone that is not on your LAN. Receives the alert's title/text/severity, the VRChat user ID and display name it concerns, your group ID, and the instance. It does **not** receive credentials, session tokens, or any file from your machine. Sent over HTTPS but **not yet end-to-end encrypted**, so the relay operator is technically able to read alert contents. | **Only** after you enable the mobile companion, pair a device, *and* accept an on-screen disclosure naming this host. Declining keeps the companion working over LAN/FCM. | Mobile is **off by default** (`mobile_enabled = false`); decline the disclosure; point `mobile_relay_endpoint` at your own server; or blank it for LAN/FCM only. Consent is per-endpoint and re-asked if it changes. |
